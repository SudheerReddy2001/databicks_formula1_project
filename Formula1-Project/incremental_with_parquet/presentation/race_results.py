...
drivers_df = spark.read.parquet(f"{process_path}/{v_file_date}/driver") \
...
constructors_df = spark.read.parquet(f"{process_path}/{v_file_date}/constructors") \
.withColumnRenamed("name", "team")

# COMMAND ----------

circuits_df = spark.read.parquet(f"{process_path}/{v_file_date}/circuits") \
.withColumnRenamed("location", "circuit_location")

# COMMAND ----------

races_df = spark.read.parquet(f"{process_path}/{v_file_date}/races") \
.withColumnRenamed("name", "race_name") \
.withColumnRenamed("race_timestamp", "race_date") 

# COMMAND ----------

results_df = spark.read.parquet(f"{process_path}/results") \
...
overwrite_partition(final_df, 'f1_presentation_inc', 'race_results', 'race_id')
