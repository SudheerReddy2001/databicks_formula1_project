...
races_with_ingestion_date_df = add_ingestion_date(races_with_timestamp_df,v_file_date)
...
races_selected_df.write.mode("overwrite").partitionBy('race_year').format("parquet").option("path",f"{process_path}/{v_file_date}/races").saveAsTable("f1_processed_inc.races")

# COMMAND ----------

# MAGIC %sql
# MAGIC  SELECT * FROM f1_processed_inc.races;
