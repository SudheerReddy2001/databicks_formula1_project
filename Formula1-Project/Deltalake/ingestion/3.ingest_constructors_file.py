...
if source_point=="adls":
    constructor_final_df.write.format("delta").mode("overwrite").save(f"{processed_folder}/constructors_adls")
elif source_point=="table":
    constructor_final_df.write.format("parquet").mode("overwrite").saveAsTable("deltalake_processed.constructors")
